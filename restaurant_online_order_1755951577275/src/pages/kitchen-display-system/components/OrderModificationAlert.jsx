...
function OrderModificationAlert({ orders, onDismiss, className = '' }) {
  ...
  return (
    <div
      className={`bg-warning-50 border border-warning-200 rounded-xl p-4 ${className}`}
    >
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-warning-600 font-body mb-4">
            The following orders have been modified by customers. Please review
            changes and update preparation accordingly.
          </p>
          ...
          <div className="space-y-3">
            {orders?.map(order => (
              <div
                key={order?.id}
                className="bg-surface rounded-lg p-4 border border-warning-200"
              >
                  ...
                  <div className="flex-1">
                        ...
                        <div key={index} className="flex items-start space-x-2">
                          ...
                          <div className="flex-1">
                            <div className="text-sm font-body text-text-primary">
                              <span className="font-body-medium capitalize">
                                {mod?.type}:
                              </span>{' '}
                              {mod?.description}
                            </div>
                    ...
                    <div className="mt-3 text-xs text-text-secondary font-body">
                      ...
                      {order?.modificationTime
                        ? new Date(order?.modificationTime).toLocaleTimeString()
                        : 'Just now'}
                    </div>
                  </div>
