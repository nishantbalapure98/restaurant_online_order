...
function OrderStatusColumn({
  ...
  const urgentOrders =
    orders?.filter(order => {
      ...
      return (
        order?.priority === 'urgent' || elapsed >= (order?.estimatedTime || 20)
      );
    })?.length || 0;
      ...
      <div className={`p-4 rounded-t-xl border-2 ${colors.header}`}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div
              className={`text-sm font-data font-data-normal px-3 py-1 rounded-full ${colors.count}`}
            >
              {orderCount}
            </div>
          </div>
        ...
        <div className="mt-2 p-3 bg-surface rounded-lg border border-border">
          <div className="grid grid-cols-2 gap-4 text-center">
            ...
            <div>
              ...
              <div className="text-xs text-text-secondary font-body">Items</div>
            </div>
          </div>
        </div>
      )}
