...
function KitchenDisplaySystem() {
  ...
  const ordersByStatus = {
    ...
    'in-progress':
      filteredOrders?.filter(order => order?.status === 'in-progress') || [],
      ...
      <div className="pt-20 p-4 lg:p-6">
        ...
        <StationFilter
          ...
          orderCounts={{
            all: orders?.length || 0,
            grill:
              orders?.filter(order => order?.station === 'grill')?.length || 0,
            fryer:
              orders?.filter(order => order?.station === 'fryer')?.length || 0,
            salad:
              orders?.filter(order => order?.station === 'salad')?.length || 0,
            beverages:
              orders?.filter(order => order?.station === 'beverages')?.length ||
              0,
          }}
        />
