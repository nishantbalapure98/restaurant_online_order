...
function ProfileSection({ userProfile, setUserProfile }) {
  ...
  const validateForm = () => {
    ...
    if (
      editForm?.phone &&
      !/^[+]?[1-9]?\d{10,14}$/.test(editForm?.phone?.replace(/[^\d]/g, ''))
    ) {
            ...
            {dietaryOptions?.map(option => (
              ...
              >
                <input
                  type="checkbox"
                  checked={
                    editForm?.dietaryPreferences?.includes(option?.id) || false
                  }
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-text-secondary">
                  Order updates, promotions, and news
                </p>
              </div>
            </div>
            <input
